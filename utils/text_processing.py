...
def summarize_conv_hist_with_gpt4o(matching_convs):
    try:
        ## Set the API key
        client = OpenAI(api_key=OPENAI_API_KEY)
        ...
        completion = client.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "Your purpose is to summarize a conversation between two entities into a short and concise summary of the conversation."},
                {"role": "user", "content": prompt}
            ]
        )
        return completion.choices[0].message.content.strip()
    except Exception as e:
        print(f"Error summarizing conversation history with GPT-4o: {e}")
        return "Sorry, I couldn't summarize the conversation history."    
